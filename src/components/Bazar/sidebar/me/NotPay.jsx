...
const NotPay = ({ refreshProducts }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.post(
          `${Config.baseUrl}/catalogue/product_not_pay_me/`,
          {},
          {
            headers: {
    ...
    fetchProducts();
  }, [jwtToken, refreshProducts]);

  const handlePayment = async (productId) => {
    try {
      const response = await axios.post(
        `${Config.baseUrl}/catalogue/payment/api/`,
  ...
  return (
    <div>
          ...
          <div key={product.id} className={styles.boxCardSingle}>
            ...
            <div className={styles.buttonContainer}>
              <button
                ...
                onClick={() => handlePayment(product.upc)}
              >
                پرداخت کنید
              </button>
              <button
      ...
      <ConfirmDialog
        message="آیا می‌خواهید آگهی خود را حذف کنید؟"
        isOpen={isDialogOpen}
