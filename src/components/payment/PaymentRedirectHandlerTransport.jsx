...
const PaymentRedirectHandlerTransport = () => {
  ...
  useEffect(() => {
    ...
    if (status === 'OK' && authority) {
      axios.get(`${Config.baseUrl}/transport/payment/verify/`, {
        params: {
          authority,
          status
        },
        ...
        if (response.data.status === 'success') {
          setMessage('پرداخت با موفقیت انجام شد. در حال هدایت به صفحه حمل و نقل...');
          setTimeout(() => {
            navigate('/transport'); // تغییر مسیر به صفحه پیشنهادات
          }, 2000); // نمایش پیام به مدت 3 ثانیه قبل از هدایت
        } else {
...
export default PaymentRedirectHandlerTransport;
